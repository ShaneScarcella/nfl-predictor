...
# Load the pre-trained model and data files when the application starts.
model = joblib.load('models/model.pkl')
try:
    # games_df is used for the main AI model predictions.
    games_df = pd.read_csv('data/games.csv', sep=r'\s*,\s*', engine='python')
    games_df.columns = games_df.columns.str.strip()

    # team_avg_stats_df contains the pre-calculated stats for the custom engine.
...
def get_predictions():
    ...
    if not played_games.empty:
        ...
        weekly_profit = 0; bet_amount = 100
...
@app.route('/custom_predict', methods=['POST']) # Endpoint for the Custom Matchup Engine
def custom_predict():
    """
    Takes user-defined weights and predicts a winner based on a simple
    weighted scoring algorithm using our pre-processed team stats.
    """
    if team_avg_stats_df.empty:
        return jsonify({'error': 'Team average stats not loaded.'})

    # Extract the matchup details and weights from the request body.
    ...
    # Retrieve the pre-calculated, week-by-week average stats for both teams.
    ...
    if home_stats.empty or away_stats.empty:
        return jsonify({'error': 'Could not find team stats for this matchup. Data might be missing for Week 1.'})

    home_score, away_score = 0, 0
    breakdown = [] # To explain the score to the user.

    # The scoring algorithm applies the user-defined weights to team stat comparisons.
    # Offensive Yards: More is better.
    if home_stats['avg_off_yards'].iloc[0] > away_stats['avg_off_yards'].iloc[0]:
        home_score += weights['offense']
        breakdown.append(f"+{weights['offense']} to {home_team} for more yards/game")
    else:
        away_score += weights['offense']
        breakdown.append(f"+{weights['offense']} to {away_team} for more yards/game")

    # Defensive Yards: Fewer allowed is better.
    if home_stats['avg_def_yards_allowed'].iloc[0] < away_stats['avg_def_yards_allowed'].iloc[0]:
        home_score += weights['defense']
        breakdown.append(f"+{weights['defense']} to {home_team} for fewer yards allowed/game")
    else:
        away_score += weights['defense']
        breakdown.append(f"+{weights['defense']} to {away_team} for fewer yards allowed/game")

    # Turnovers: Fewer is better.
    if home_stats['avg_turnovers'].iloc[0] < away_stats['avg_turnovers'].iloc[0]:
        home_score += weights['turnovers']
        breakdown.append(f"+{weights['turnovers']} to {home_team} for fewer turnovers/game")
    else:
        away_score += weights['turnovers']
        breakdown.append(f"+{weights['turnovers']} to {away_team} for fewer turnovers/game")
...
def get_current_week_info():
    """Uses the nflreadpy library to determine the current season and week."""
    try:
        current_season = nfl.get_current_season()
        current_week = nfl.get_current_week()
        return jsonify({'season': current_season, 'week': current_week})
    except Exception as e:
        # Fallback in case the library has an issue
